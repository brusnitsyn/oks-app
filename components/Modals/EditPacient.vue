...
const { data: pacient, refresh: refreshEdit } = await useAPI(`/api/pacient/${props.pacientId}/edit`)
...
function handleSubmit() {
  formRef.value?.validate(async (errors) => {
    if (!errors) {
      loading.value = true
      const { data, status } = await useAPI(`/api/pacient/${model.value.id}`, {
        method: 'POST',
        body: model.value,
      })
...
<template>
  <NModal v-model:show="show" :mask-closable="false" preset="card" class="max-w-screen-sm xl:max-w-screen-md min-h-[742px]" title="Редактирование пациента">
    <NForm ref="formRef" :disabled="useSanctumAuth().isOperator" :rules="rules" :model="model" @submit.prevent="() => onSubmit(handleSubmit)">
      <NTabs type="segment">
        <NTabPane display-directive="show" name="main" tab="Персональная информация">
        ...
        <NTabPane v-if="model.info != null" display-directive="show" name="info" tab="Информация по заболеванию">
